...
const SHAPComparisonChart = ({ data, selectedFeatureName }) => {
  ...
  leagues.forEach((league, index) => {
    ...
    if (leagueData) {
      leagues[index] = leagueData.name;
    }
  });

  const classes = ["Victoria del equipo visitante", "Empate", "Victoria del equipo local"];

  const generateChartForClass = (className) => {
    const scatterData = [];

    data.forEach((leagueData, leagueIndex) => {
      const classInfo = leagueData.classes.find(cls => cls.class === className);
      if (classInfo) {
        ...
        classInfo.shap_values.forEach((shapVal, idx) => {
          ...
          scatterData.push({
            ...
            tooltipData: { featureVal, shapVal, league: leagueData.league },
            itemStyle: { color }
          });
        });
      }
    ...
    return (
      <ReactECharts
        ...
        option={{
          title: {
            text: `${selectedFeatureName} - ${className}`,
            left: 'center',
            textStyle: {
              fontSize: 20,
              fontWeight: '800',
  ...
  return (
    <div>
      {classes.map((className) => generateChartForClass(className))}
    </div>
  );
};
